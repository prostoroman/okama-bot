...
#### 🎯 Естественный язык
Бот понимает запросы на естественном языке и автоматически конвертирует их в команды:
- **"Проанализируй Apple"** → `/info AAPL.US`
- **"Сравни Tesla и Microsoft"** → `/compare TSLA.US MSFT.US`
- **"Портфель из золота и серебра"** → `/portfolio XAU.COMM:0.5 XAG.COMM:0.5`

#### 🔍 Автоматическое распознавание намерений
- **single_asset_info** - Анализ одного актива
- **asset_comparison** - Сравнение активов
- **portfolio_analysis** - Анализ портфеля
- **inflation_info** - Данные по инфляции
- **macro_info** - Макроэкономические показатели
...
- **Месячные цены (close_monthly)** - Месячные цены для анализа долгосрочных трендов
- **Накопленная доходность** - График роста портфеля
- **Корреляционная матрица** - Тепловая карта корреляций между активами
...
#### 🎨 Стили графиков
- **Nordic** - Современный минималистичный стиль с плавными линиями (сплайны)
- Автоматическая интерполяция для красивого отображения
- Поддержка CJK шрифтов для китайских символов

### 🤖 AI-сервисы и анализ

#### 🧠 YandexGPT (основной AI)
- **Финансовый анализ** - Профессиональные выводы по графикам и данным
- **Консультации** - Ответы на финансовые вопросы
- **Оптимизация портфеля** - Рекомендации по улучшению
- **Естественный язык** - Понимание запросов на русском и английском
...
- **Базовый анализ** - Когда AI сервисы недоступны
- **Структурированные выводы** - Стандартные рекомендации
...
- **PORTFOLIO_1**, **PORTFOLIO_2**, **PORTFOLIO_3** и т.д.
...
- **YandexGPT** - AI анализ и консультации
...
- **Inline кнопки** - Быстрый доступ к функциям
- **Клавиатуры** - Удобная навигация
- **Callback обработчики** - Интерактивные действия
...
### ✅ Команда /list улучшена - добавлены кнопки
...
- При нажатии на кнопку вызывается тот же функционал, что и при команде `/list <код>`
...
/list [код]          # Список пространств имен или символы в пространстве
```

**Команда /list с кнопками:**
- `/list` - показывает все доступные пространства имен с удобными кнопками
...
| `/list` | Просмотр пространств имен | `/list` |
...
- **`/info [символ]`** - Детальная информация об активе с графиками и AI анализом
- **`/portfolio`** - Создание и анализ портфеля с весами
- **`/compare`** - Сравнение активов и портфелей
- **`/my`** - Просмотр всех сохраненных портфелей
- **`/list [код]`** - Просмотр пространств имен или символов
- **`/gemini_status`** - Проверка статуса AI сервисов

### 🌍 Поддерживаемые рынки (всего 15+ бирж)
- **🇺🇸 США** (.US) - NYSE, NASDAQ, AMEX
- **🇷🇺 Россия** (.MOEX) - Московская биржа
- **🇨🇳 Китай** (.SH/.SZ/.BJ/.HK) - Шанхай, Шэньчжэнь, Пекин, Гонконг
- **🇬🇧 Великобритания** (.LSE) - Лондонская биржа
- **🇩🇪 Германия** (.XETR) - Франкфуртская биржа
- **🇫🇷 Франция** (.XFRA) - Парижская биржа
- **🇳🇱 Нидерланды** (.XAMS) - Амстердамская биржа
- **💱 Валюты** (.FX) - FOREX рынок
- **🛢️ Товары** (.COMM) - Товарные активы
- **₿ Криптовалюты** (.CC) - Криптовалютный рынок
...
### 📈 Финансовые метрики (15+ показателей)
- **Доходность**: Total Return, Annual Return, CAGR
- **Риск**: Volatility, Max Drawdown, VaR, CVaR
- **Эффективность**: Sharpe Ratio, Sortino Ratio
- **Динамика**: Rolling CAGR, Rolling Volatility, Rolling Sharpe
- **Дивиденды**: Dividend Yield, Dividend History
- **Оценка**: Price-to-Earnings, Price-to-Book

### 🎨 Визуализация
- **Дневные графики** - Скорректированные дневные цены
- **Месячные графики** - Долгосрочные тренды
- **Накопленная доходность** - Рост портфеля
- **Корреляционные матрицы** - Тепловые карты
- **Nordic стиль** - Современный минималистичный дизайн
- **Сплайны** - Плавные линии с интерполяцией

### 💼 Управление портфелями
- **Создание портфелей** - С указанием весов
- **Автоматические символы** - PORTFOLIO_1, PORTFOLIO_2, etc.
- **Сохранение контекста** - Все атрибуты портфелей
- **Сравнение портфелей** - С активами и между собой
- **Просмотр портфелей** - Команда `/my`

### 🔧 Технические особенности
- **Модульная архитектура** - Разделение по сервисам
- **Thread-safe операции** - Безопасная многопоточность
- **In-memory storage** - Быстрый доступ к данным
- **Graceful degradation** - Работа при недоступности сервисов
- **Автоматическая нормализация** - Конвертация в тикеры Okama
