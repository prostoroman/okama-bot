...
Используй строгий профессиональный тон, глубокую аналитику и структуру."""
        ...
        self.custom_prompts = {
            'chart': {
                'role': 'финансовый аналитик уровня ведущих инвестиционных банков',
            ...
            'data_comparison': {
                ...
                'requirements': [
                    'СРАВНИТЕЛЬНЫЙ АНАЛИЗ АКТИВОВ: Сравни каждый актив по всем метрикам, выдели лидеров и аутсайдеров',
                    ...
                    'СИЛЬНЫЕ И СЛАБЫЕ СТОРОНЫ: Выдели преимущества каждого актива, укажи на недостатки и риски'
                ],
                'format': 'Структурированный отчет с подзаголовками, четкий язык, профессиональные термины с пояснениями',
                'disclaimer': 'Добавь юридически правильный дисклеймер о том, что анализ не учитывает макроэкономические факторы, политическую ситуацию и пр, не является инвестиционной рекомендацией'
            },
            
            'portfolio': {
                'role': 'ведущий портфельный аналитик уровня крупнейших инвестиционных фондов',
                ...
                'requirements': [
                    ...
                    'СТРАТЕГИЧЕСКИЕ РЕКОМЕНДАЦИИ: Оцени соответствие портфеля инвестиционным целям',
                    'РИСК-МЕНЕДЖМЕНТ: Оцени концентрационные риски, проанализируй чувствительность к рыночным шокам'
                ],
                'format': 'Структурированный отчет с четкими подзаголовками: 📊 Анализ текущего портфеля, ⚖️ Анализ состава портфеля, 🔄 Рекомендации по ребалансировке, 🎯 Стратегические рекомендации, ⚠️ Риск-менеджмент',
                'style': 'профессиональный язык, конкретные цифры и обоснованные выводы',
                'disclaimer': 'Добавь дисклеймер о том, что анализ не является инвестиционной рекомендацией'
            }
        }
    ...
    def _build_prompt(self, analysis_type: str, custom_data: str = "") -> str:
        ...
        # Добавляем специфичную роль и задачу
        prompt_parts.append(f"Твоя роль: {prompt_config['role']}")
        prompt_parts.append(f"Твоя задача: {prompt_config['task']}")
        ...
        if 'requirements' in prompt_config:
            ...
            for i, requirement in enumerate(prompt_config['requirements'], 1):
                prompt_parts.append(f"\n{i}. **{requirement}**")
        
        # Добавляем формат ответа
        if 'format' in prompt_config:
            prompt_parts.append(f"\n**ФОРМАТ ОТВЕТА:**")
            prompt_parts.append(prompt_config['format'])
        
        # Добавляем стиль
        if 'style' in prompt_config:
            prompt_parts.append(f"\n**СТИЛЬ:**")
            prompt_parts.append(prompt_config['style'])
        
        # Добавляем ограничения
        if 'restrictions' in prompt_config:
            prompt_parts.append(f"\n**ВАЖНО:** {prompt_config['restrictions']}")
        
        # Добавляем дисклеймер
        if 'disclaimer' in prompt_config:
            prompt_parts.append(f"\n**ДИСКЛЕЙМЕР:** {prompt_config['disclaimer']}")
